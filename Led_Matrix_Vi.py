...
SIZE = 22
PREVIEW_SIZE = 8
MAX_COLS = 1024   # Vùng nhớ lớn để giữ LED khi ra ngoài
...
class LedMatrix:
    ...
    def build_matrix(self):
        ...
        for r in range(ROWS):
            ...
            for c in range(self.cols):
                obj = self.canvas.create_oval(c*SIZE, r*SIZE, (c+1)*SIZE, (r+1)*SIZE, fill="white", outline="gray")
                self.canvas.tag_bind(obj, "<Button-1>", lambda e, rr=r, cc=c: self.toggle(rr, cc))
                row.append(obj)
            self.led.append(row)
    ...
    # ====================== SHIFT - ULTIMATE ĐÃ SỬA (LED KHÔNG MẤT) ======================
    def shift(self, dx, dy):
        ...
        if self.shift_mode == "wrap":
            ...
            # Ultimate mode - Giữ LED khi ra ngoài biên
            for r in range(ROWS):
